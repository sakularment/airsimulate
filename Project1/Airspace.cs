...
namespace Project1
{
    ...
    class Airspace
    {
        ...
        //批量添加新飞机 调用添加Add_newaircrafts()方法，有并发的话，溢出的判断条件必须为原子操作
        ...
        static void Main(string[] args){
            ...
            while (true)
            {
                ...
                if (s == "exit") { break; }
                    //新建飞机指令
                else if (s == "new") { string s2 = Console.ReadLine(); asp.Add_newaircrafts(int.Parse(s2)); Console.WriteLine(s2+"架飞机已经创建"); }
                    //显示飞机信息指令
                    ...
                    //帮助
                else if (s == "help") { Console.WriteLine("new+数字：创建飞机\ninfo获取飞机信息\ngo+数字时间流逝n秒\ntrac获取所有飞机的轨迹"); }
                    //报错
